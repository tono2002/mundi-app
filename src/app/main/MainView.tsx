...
export default function MainView({ matches, bars }: { matches: Match[]; bars: BarFromDB[] }) {
  ...
  const visibleBars = useMemo<MapBar[]>(() => {
    ...
    return bars
      ...
      .map((b) => ({ id: b.id, name: b.name, description: b.description, address: b.address, phone: b.phone, lat: b.lat, lng: b.lng }))
  }, [bars, selectedMatch])
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-20 flex items-center h-14 bg-white shadow-md px-3 gap-2 shrink-0">
        <Link href="/" className="shrink-0 flex items-center justify-center w-9 h-9 rounded-xl hover:bg-green-50 transition-colors" title="Inicio">
          <svg viewBox="0 0 24 24" className="w-6 h-6 text-green-700" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        {showSearch ? (
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            />
            <button onClick={() => { setShowSearch(false); setSearch('') }} className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100">
              <X className="w-4 h-4" />
            </button>
          </div>
        ) : (
          <button onClick={() => setShowSearch(true)} className="shrink-0 flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-xl transition-colors">
        ...
        {/* Match carousel */}
        <div className="flex-1 flex items-center gap-2 overflow-x-auto py-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {displayMatches.map((match) => {
            ...
            const barCount = bars.filter((b) => b.bar_matches.some((bm) => bm.match_id === match.id)).length
            return (
              <button
                key={match.id}
                ...
                className={[
                  ...
                  isSelected ? 'bg-green-700 text-white border-green-700 shadow-md'
                    : isFav ? 'bg-green-50 border-green-200 text-gray-800 hover:border-green-400'
                    : 'bg-white border-gray-200 text-gray-700 hover:border-gray-300',
                ].join(' ')}
              >
                {isFav && !isSelected && <span className="text-amber-400">⭐</span>}
          ...
          {displayMatches.length === 0 && <span className="text-sm text-gray-400 px-2 shrink-0">Sin resultados</span>}
        </div>
      </div>
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {selectedMatch && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
            <div className="bg-white rounded-2xl shadow-lg px-4 py-2 flex items-center gap-3 pointer-events-auto">
              ...
              <button onClick={() => { setSelectedMatch(null); setSelectedBar(null) }} className="text-gray-300 hover:text-gray-500 ml-1">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        {/* ── BAR INFO CARD ── */}
        {selectedBar && (
          <div className="absolute bottom-4 left-4 z-20 bg-white rounded-2xl shadow-xl overflow-hidden" style={{ width: '300px', maxWidth: 'calc(100vw - 32px)' }}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              ...
              <button onClick={() => setSelectedBar(null)} className="text-gray-300 hover:text-gray-500 ml-2 shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-4 flex flex-col gap-2.5">
              ...
              <a
                ...
                className="w-full py-2 bg-green-700 text-white text-xs font-bold rounded-xl hover:bg-green-800 transition-colors flex items-center justify-center gap-1.5"
              >
